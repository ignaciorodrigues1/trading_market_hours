...
export default function MarketCard({ market, localTime, openTime, closeTime, selectedTime }: any) {
  // Calcular un porcentaje de cierre del mercado (este es un ejemplo, ajusta la lógica según tu caso)
  const marketProgress = ((new Date().getHours() - market.open) / (market.close - market.open)) * 100;

  return (
    <Card key={market.name} className="shadow-lg">
      <CardHeader>
        <h3 className="text-xl font-semibold flex items-center">
          <Flag code={market.flag} className="w-8 h-8 mr-2" />
          {market.name}
        </h3>
